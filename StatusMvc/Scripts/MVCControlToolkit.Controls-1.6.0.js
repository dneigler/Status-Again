...
function DateInputChanged(evt, cid, update, force) {
...
function DataGrid_Field(original, current, validationType) {
    ...
    if (this.Original != null) {
        var attr = $(this.Original).attr('element-type');
...
function DataGrid_ResetRow(itemRoot) {
    ...
    if (fields == null) {
        ...
        temp.find('input:not([elementispart])').each(function (i) {
            fields.AddOriginal(this.id, this);
        });
        temp.find('textarea:not([elementispart])').each(function (i) {
            fields.AddOriginal(this.id, this);
        });
        temp.find('select:not([elementispart])').each(function (i) {
            fields.AddOriginal(this.id, this);
        });
        temp.find('[element-type]').each(function (i) {
            fields.AddOriginal(this.id, this);
        });
...
function MvcControlsToolkit_DisplayEdit_Init(p0, p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12, p13, p14) {
    $(document).ready(function () {
        ...
        if (p13 != null) tTb.datepicker(p13);
        tTb.bind('pblur', function () {
            MvcControlsToolkit_DisplayEdit_Blur(p0, p1, p2, p7, p8, p9, p5, p6, p3, p4,
                           p10, p12, p14);
            return false;
        });
        try {
...
decimalSeparator, digitSeparator, validationType, nullDisplayText, inputType) {
    var fieldElement = document.getElementById(fieldId);
    if (fieldElement == null) return;
    var value = fieldElement.value;
    if (inputType == "select") {
    ...
    if (MvcControlsToolkit_Validate(fieldId, validationType)) {
        $('#' + fieldId).hide();
        $('#' + companionId).show();
