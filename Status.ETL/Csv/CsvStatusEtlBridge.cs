            ...
            try
            {
                ...
                foreach (var teamG in grpTeams)
                {
                    ...
                    if (team == null)
                    {
                        ...
                        var teamCsv = grpTeams.Single().Single();
                        var lead = this.ResourceRepository.GetResourcesByName(teamCsv.TeamLead);
                        Team t = new Team() { Name = teamG.Key, Lead = (Employee)lead };
                ...
                projectNames.ToList().ForEach(p =>
                {
                    this.ProjectRepository.AddProject(new Project() { Name = p, Caption = p });
                });
                ...
                grpTopics.ToList().ForEach(
                    gt =>
                    {
                        ...
                        else
                        {
                            ...
                            gt.ToList().ForEach(item =>
                            {
                                this.TopicRepository.AddTopic(new Topic() { Caption = item.Note });
                            });
                        }
                ...
                Resource resource = new Resource() { EmailAddress = "t@t.com", FirstName = "FN", LastName = "LN" };
                this.ResourceRepository.AddResource(resource);
                        ...
                        gsd.ToList().ForEach(statusReportItem =>
                                                 {
                                                     ...
                                                     statusItem.Notes.Add(new Note()
                                                                              {
                                                                                  AuditInfo = new AuditInfo(resource),
